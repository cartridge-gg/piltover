...
fn deploy_with_owner_and_state(
    owner: felt252, state_root: felt252, block_number: i128, block_hash: felt252,
) -> (IAppchainDispatcher, EventSpy) {
    let contract = snf::declare("appchain").unwrap();
    let calldata = array![owner, state_root, block_number.into(), block_hash];
    let (contract_address, _) = contract.deploy(@calldata).unwrap();
...
fn get_invalid_state_update() -> Array<felt252> {
    let felts = array![
        1,
        2,
        3, //We add these three values to match the orginal way that snos output is sent to piltover
...
fn get_invalid_output() -> Span<felt252> {
    let snos_output = get_invalid_state_update();
    let snos_output_hash = poseidon_hash_span(snos_output.span());
    let felts = array![1, 2, 3, 4, snos_output_hash];
    felts.span()
}
...
fn update_state_invalid_block_number() {
	let initial_block_number: i128 = -66;
